...
public class ArvoreAVL<T> extends ArvoreBinaria<T>{
    ...
    public void adicionar(T novoValor) {
        this.raiz = adicionarRecursivamente(this.raiz, novoValor);
    }
    ...
    private No<T> adicionarRecursivamente(No<T> noAtual, T novoValor) {
        ...
        int comparacao = this.comparador.compare(novoValor, noAtual.getValor());
    ...
    protected No<T> removerRecursivo(No<T> no, T valor) {
        ...
        if (comparacao < 0) {
            ...
            no.setValor(this.encontrarMinimo(no.getFilhoDireita()).getValor());
